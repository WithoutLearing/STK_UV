...
namespace BCM检测工装
{
    public partial class frmRecycling : Form
    {
            ...
            if (e.KeyChar == '\r')
            {
                ...
                if (QueryData(PublicData.ProductCode))//查询到数据库中已存在胶水信息
                {
                    if (PublicData.ProductSeqNumber == 1)
                    {
                        ...
                        //当胶水常温时间超过（60-2）h或者次数已经到达3次,就没有必要回箱了
        ...
        private void ShowData()
        {
            ...
            string cmdText = "select 第一次回箱扫码时间,第二次回箱扫码时间,操作人,胶水序号, 胶水ID,胶水状态,胶水回温次数, 胶水回温计时s,胶水常温计时s from ProductInformation";
        ...
        private void UpdateData(int i, int times)
        {
            if (times == 1)
            {
                ...
                string cmdText = "UPDATE ProductInformation SET 第一次回箱扫码时间=@DateTime8,操作人=@Name,胶水序号=@Seqnum,胶水状态=@State,胶水回温次数=@Numbers,胶水回温计时s=@BackTimes,胶水常温计时s=@Times WHERE 胶水ID=@QrCode";
                //给参数赋值
                SQLiteParameter[] parameter = new SQLiteParameter[] {
                                                                    new SQLiteParameter("@DateTime8", DateTime.Now),
            ...
            {
                //定义sql语句文本,更新表中二维码相同的数据
                string cmdText = "UPDATE ProductInformation SET 第二次回箱扫码时间=@DateTime9,操作人=@Name,胶水序号=@Seqnum,胶水状态=@State,胶水回温次数=@Numbers,胶水回温计时s=@BackTimes,胶水常温计时s=@Times WHERE 胶水ID=@QrCode";
                //给参数赋值
                SQLiteParameter[] parameter = new SQLiteParameter[] {
                                                                    new SQLiteParameter("@DateTime9", DateTime.Now),
                                                                    new SQLiteParameter("@Name", PublicData.LoginUsername),
                                                                    new SQLiteParameter("@Seqnum", PublicData.GlueSeqNumberList[i]),
                                                                    new SQLiteParameter("@QrCode", PublicData.GlueIDList[i]),
                                                                    new SQLiteParameter("@State", PublicData.GlueStateList[i]),
